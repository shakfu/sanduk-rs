...
4. **CLI, agents, recipes, kits.** Done. `run`, `build`, `shell`, `ps`, `stop`, `clean`, `destroy`, `system` and `list`; the assistant verbs are step 5. Checked three ways against Python sanduk 0.3.1:
   ...
   Found on the live run: the macOS firewall prompts for the unsigned binary the first time the relay listens. `firewall_warning` detects only an explicit Block entry, so an unattended run would hang on the prompt; TODO.md records the same gap for Python. A debug build's ad-hoc signature changes on every build, which is believed to re-prompt (unmeasured).
